...
export function registerIpcHandlers() {
  ...
  ipcMain.handle("fs:readAppDataJson", async (_event: any, filename: string) => {
    try {
  ...
  ipcMain.handle("fs:writeAppDataJson", async (_event: any, filename: string, content: string) => {
    try {
      if (typeof filename !== "string") {
        throw new Error("Filename must be a string");
      }
  ...
  ipcMain.handle("fs:readTextFile", async (_event: any, filePath: string) => {
    try {
      if (typeof filePath !== "string") {
        throw new Error("Path must be a string");
      }
  ...
  ipcMain.handle("fs:writeTextFile", async (_event: any, filePath: string, content: string) => {
    try {
      if (typeof filePath !== "string") {
        throw new Error("Path must be a string");
      }
  ...
  ipcMain.handle("fs:mkdir", async (_event: any, dirPath: string, options?: { recursive?: boolean }) => {
    try {
  ...
  // Dialog handlers
  ipcMain.handle("dialog:open", async (_event: any, options?: {
    title?: string;
  ...
  ipcMain.handle("openExternal", async (_event: any, url: string) => {
    try {
